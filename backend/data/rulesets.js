...
export const Rulesets = {
  ...
  obtainablemisc: {
    ...
    onChangeSet(set) {
      ...
      if (set.moves) {
        const hasMove = {}; // This object will hold keys of moves, where the value is always `true`.
        for (const moveId of set.moves) {
          const move = this.dex.moves.get(moveId);
  ...
  forceofthefallenmod: {
    ...
    onFaint(target) {
      ...
      for (const ally of allies) {
        ally.moveSlots = ally.baseMoveSlots = [
          ...ally.baseMoveSlots,
    ...
    onResidual(pokemon) {
      ...
      const abilitySlot =
        Object.keys(oldPokemon.abilities).find(
          (x) => oldPokemon.abilities[x] === pokemon.set.ability
        ) || "0";
